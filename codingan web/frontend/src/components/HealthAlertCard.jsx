...
const getBPMRange = (age) => {
  if (age === null || age === undefined)
    return { min: 60, max: 100, label: "Dewasa" };
...
export const HealthAlertCard = ({
  ...
  const hasWarning =
    bpmAnalysis.status !== "normal" && bpmAnalysis.status !== "unknown";
  const hasSpo2Warning =
    spo2Analysis.status !== "normal" && spo2Analysis.status !== "unknown";
        ...
        <div className={`alert-item ${bpmAnalysis.color}`}>
          ...
          <div className="alert-info">
            ...
            <div className={`alert-status ${bpmAnalysis.color}`}>
              {bpmAnalysis.status === "critical" && (
                <i className="fas fa-exclamation-triangle"></i>
              )}
              {bpmAnalysis.status === "high" && (
                <i className="fas fa-arrow-up"></i>
              )}
              {bpmAnalysis.status === "low" && (
                <i className="fas fa-arrow-down"></i>
              )}
              {bpmAnalysis.status === "normal" && (
          ...
          <div className="alert-info">
            <div className="alert-label">
              ...
              <span className="current-value">
                {isFirebaseConnected && spo2 > 0 ? `${spo2}%` : "--"}
              </span>
            </div>
            <div className={`alert-status ${spo2Analysis.color}`}>
              {spo2Analysis.status === "critical" && (
                <i className="fas fa-exclamation-triangle"></i>
              )}
              {spo2Analysis.status === "low" && (
                <i className="fas fa-arrow-down"></i>
              )}
              {spo2Analysis.status === "normal" && (
                <i className="fas fa-check-circle"></i>
              )}
        ...
        {(hasWarning || hasSpo2Warning) && (
          <div
            className={`warning-banner ${
              bpmAnalysis.severity === "high" ||
              spo2Analysis.severity === "high"
                ? "critical"
                : "warning"
            }`}
          >
            ...
            <div className="warning-text">
              ...
              <span>
                {bpmAnalysis.severity === "high" &&
                  "Detak jantung dalam kondisi kritis. "}
                {spo2Analysis.severity === "high" &&
                  "Saturasi oksigen sangat rendah, butuh terapi oksigen. "}
                {(bpmAnalysis.severity === "medium" ||
                  spo2Analysis.severity === "medium") &&
                  bpmAnalysis.severity !== "high" &&
                  spo2Analysis.severity !== "high" &&
